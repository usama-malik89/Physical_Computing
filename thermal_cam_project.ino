...
// Anything else!
  ...
  Adafruit_ST7735 tft = Adafruit_ST7735(TFT_CS,  TFT_DC, STMPE_CS);
...
void drawpixels(float *p, uint8_t rows, uint8_t cols, uint8_t boxWidth, uint8_t boxHeight, boolean showVal) {
  ...
  for (int y=0; y<rows; y++) {
    for (int x=0; x<cols; x++) {
      ...
      uint16_t color;
      color = val * 2;
      tft.fillRect(2 +boxWidth * x, boxHeight * y, boxWidth, boxHeight, camColors[colorIndex]);
